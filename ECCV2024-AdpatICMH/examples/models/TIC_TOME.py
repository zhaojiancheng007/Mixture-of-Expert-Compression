...
from layers.layers import SwinTransformerBlock, window_partition, window_reverse
...
def _compute_tome_ops(module, tokens, h, w, window_size=None):
    ...
    if window_size is None:
        ...
        attn_can_merge = ((h_out % window_size) == 0 and (w_out % window_size) == 0)
    if (not cfg["merge_mlp"]) and ((not cfg["merge_attn"]) or (not attn_can_merge)):
        return do_nothing, do_nothing, do_nothing, do_nothing, h, w
    ...
    if cfg["merge_attn"] and attn_can_merge:
...
def _swin_forward_tome(self, x, x_size):
    h, w = x_size
    b, l, c = x.shape
    if l != h * w:
        return self._tome_orig_forward(x, x_size)

    shortcut = x
    norm1 = self.norm1(x)
    m_a, m_m, u_a, u_m, h_a, w_a = _compute_tome_ops(self, norm1, h, w, window_size=self.window_size)

    x_attn = m_a(norm1, mode=self._tome_cfg["reduce"]).reshape(b, h_a, w_a, c)
    can_shift = self.shift_size > 0 and min(h_a, w_a) > self.shift_size
    if can_shift:
    ...
    shifted_x = window_reverse(attn_windows, self.window_size, h_a, w_a)

    if can_shift:
        x_attn = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
    else:
        x_attn = shifted_x
    x_attn = x_attn.view(b, h_a * w_a, c)
    x_attn = u_a(x_attn).reshape(b, l, c)
    ...
    mlp_out = u_m(mlp_out).reshape(b, l, c)
    x = x + self.drop_path(mlp_out)
    return x, out_vis


class TIC_TOME(TIC):
    """TIC with ToMe acceleration injected into SwinTransformerBlock."""

    def __init__(self, N=128, M=192, args=None, **kwargs):
    ...
    def _apply_tome_patch(self):
        ...
        for name, module in self.named_modules():
            ...
            module.forward = MethodType(_swin_forward_tome, module)
            patched += 1
