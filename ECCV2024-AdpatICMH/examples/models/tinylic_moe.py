"""
TinyLIC_MoE -- TinyLIC with optional Mixture-of-Experts on the FFN
inside every ResViTBlock.

MoE is applied to g_a / g_s pathway ResViTBlocks only.
h_a / h_s (hyper-encoder / hyper-decoder) always use standard Mlp.

Usage:
    model = TinyLIC_MoE(N=128, M=320, args=args)
...
from layers.layers_tinylic_moe import ResViTBlock, MultistageMaskedConv2d
...
class TinyLIC_MoE(nn.Module):
    r"""TinyLIC with optional Mixture-of-Experts.

    Based on Ming Lu and Zhan Ma, "High-Efficiency Lossy Image Coding Through
    Adaptive Neighborhood Information Aggregation".

    MoE replaces the FFN inside each NSABlock of the ResViTBlock layers in g_a
    and g_s.  h_a and h_s always use standard (dense) Mlp.

    Args:
        N (int): Number of channels.
        M (int): Number of channels in the expansion layers.
    ...
    def __init__(self, N=128, M=320, args=None):
        ...
        # ---- g_a (encoder) : MoE-enabled ResViTBlocks ----
        ...
        # ---- g_s (decoder) : MoE-enabled ResViTBlocks ----
        ...
        # ---- entropy models (identical to base TinyLIC) ----
